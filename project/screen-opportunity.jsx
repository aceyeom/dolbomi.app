// screen-opportunity.jsx — Opportunity detail. Glassy, desaturated header (text always
// legible on a frosted plate), an 신청 정보 accordion (자격·비용·신청처), and a clean plan
...
function OppHeader({ o }) {
  ...
  return (
    <div style={{ position: 'relative', height: 152, borderRadius: 'var(--r-lg)', overflow: 'hidden', marginBottom: 14,
      ...
      {/* frosted title plate — text is always crisp here */}
...
function InfoRow({ k, v, vColor = 'var(--ink)', icon, last }) {
  return (
...
function OppDetail({ o, ms, onOpenPlan, onAddTonight }) {
  ...
  return (
    <div className="tm-rise">
      ...
      {/* the detailed pacing bar */}
      <window.OppProgressBar o={o} ms={ms} />
      ...
      {/* 신청 정보 accordion — eligibility · cost · where (a main part, open by default) */}
      ...
      {/* quest path summary → opens full plan tab */}
...
window.OppDetail = OppDetail;
window.OppAccordion = Accordion;
})();
