...
// and binds it to TEMPO's live state: 6 stats sculpt the body, milestones add orbiting
...
// resolve a CSS custom property on the nearest .tempo root to a real color string
function resolveAccent(el) {
  let n = el;
  while (n && !(n.classList && n.classList.contains('tempo'))) n = n.parentElement;
...
function CreatureHero({ path = 'haechi', animal = 'ram', companion = null, stats, milestones = 0, theme = 'dark', pulseSignal = 0, level = 1, interactive = false, onCompanionTap = null }) {
  ...
  const [ready, setReady] = useState(!!window.TempoCreature);

  // wait for the ES module to attach window.TempoCreature
  useEffect(() => {
    if (window.TempoCreature) { setReady(true); return; }
    const on = () => setReady(true);
    window.addEventListener('tempo-creature-ready', on);
    const iv = setInterval(() => { if (window.TempoCreature) { setReady(true); clearInterval(iv); } }, 120);
    return () => { window.removeEventListener('tempo-creature-ready', on); clearInterval(iv); };
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      apiRef.current = window.TempoCreature.create({
