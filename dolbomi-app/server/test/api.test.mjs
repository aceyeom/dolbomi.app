...
const server = spawn(process.execPath, [join(__dirname, '..', 'index.js')], {
  env: { ...process.env, PORT: String(PORT), DOLBOMI_DB: TEST_DB },
...
try {
  ...
  // auth: good creds
  r = await fetch(`${BASE}/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ login: 'demo', password: 'dolbomi' }) });
